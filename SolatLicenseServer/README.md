...
1. **You generate a code** in the admin panel (choose masjid name + plan:
   Bulanan/Separuh Tahun/Tahunan/**Lifetime**) — a 9-digit code is created.
...
## Free demo (first install only)

On its very first launch on any device, the app automatically requests a
**10-day demo** — no code needed. This is tracked by device ID on this
server, so a masjid can't get another free demo just by reinstalling the
app; once used, that device must enter a real (paid) code to continue.

When the demo runs out, the app shows a distinct "Tempoh Percubaan Tamat"
(demo expired) screen encouraging them to subscribe — separate from the
regular "Langganan Tamat Tempoh" message shown when a *paid* subscription
expires.

Demo records aren't shown in the main license table (they're not something
you issue) — they're purely automatic and tracked internally.

## Lifetime plan

In addition to Bulanan/Separuh Tahun/Tahunan, you can issue a **Lifetime**
license — effectively never expires (set 100 years out internally). Useful
for a one-time payment deal, or for your own test devices.
